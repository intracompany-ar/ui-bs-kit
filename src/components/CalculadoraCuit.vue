...
const document = ref('');
const tipoEntidad = ref<string>('20');
...
function calculate() {
    ...
    let resto = suma % 11;
    let xyInicial: number = parseInt(tipoEntidad.value);
    ...
    if (11 - resto == 10) {// Implica que el verificador es 10, y solo pueden tener 1 digito => se cambia el xy incial
        ...
        xyInicial = Number(tipoEntidad.value)
        verificador = 0
    } else {
        xyInicial = Number(tipoEntidad.value)
        verificador = 11 - resto
    ...
    <div class="row">
        <div class="col-12">
            <div class="btn-group btn-group-sm mb-2" role="group" aria-label="Basic example">
                <input type="radio" class="btn-check" value="20" name="tipo_entidad" id="tipoEnteH" autocomplete="off"
                    v-model="tipoEntidad" checked>
                <label class="btn btn-outline-danger" for="tipoEnteH">Hombre</label>

                <input type="radio" class="btn-check" value="27" name="tipo_entidad" id="tipoEnteM" autocomplete="off"
                    v-model="tipoEntidad">
                <label class="btn btn-outline-danger" for="tipoEnteM">Mujer</label>

                <input type="radio" class="btn-check" value="30" name="tipo_entidad" id="tipoEnteS" autocomplete="off"
                    v-model="tipoEntidad">
        ...
        <div class="row">
            <div class="col-6">
                ...
                <input type="number" min="0" max="99999999" maxlength="8" class="form-control" placeholder="Documento"
                    v-model="document" id="document">
            </div>
        </div>
    </div>
</template>