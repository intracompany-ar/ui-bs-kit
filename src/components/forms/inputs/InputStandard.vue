...
arrParams.forEach(param => {
    ...
    if (paramDots[1]) {
        switch (paramDots[0]) {
            case 'min':
                min.value = Number(paramDots[1]);
                break;
            case 'max':
                max.value = Number(paramDots[1]);
                break;
        };
    } else {
        switch (param) {
...
<template>
    <div v-bind:class="props.divclass">
        ...
        <input v-bind:type="type" v-bind:maxlength="max" v-bind:max="max" v-bind:min="min" class="form-control"
            v-bind:class="{ 'form-control-sm': props.small }" v-bind:name="props.name" v-bind:value="props.value"
